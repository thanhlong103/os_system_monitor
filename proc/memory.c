...
void get_memory_info(long *total_mem, long *used_mem, long *mem_buffers, long *mem_cached, long *mem_free, long *mem_available) {
    FILE *fp = fopen("/proc/meminfo", "r");
    if (fp == NULL) {
        perror("Failed to read /proc/meminfo");
        *total_mem = *used_mem = *mem_buffers = *mem_cached = *mem_free = *mem_available = -1;
        return;
    }
    ...
    *total_mem = *mem_free = *mem_buffers = *mem_cached = *mem_available = 0;
    ...
    while (fscanf(fp, "%31s %ld kB", label, &value) != EOF) {
        ...
        if (*total_mem && *mem_free && *mem_buffers && *mem_cached && *mem_available) {
            break;
        }
    }
    fclose(fp);
